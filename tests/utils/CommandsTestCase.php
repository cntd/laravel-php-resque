...
abstract class CommandsTestCase extends TestCase {
    ...
    */
    ...
    public function forkListen($queue, $count=1, $output = null, $expire = 60 ) {
        $pid = pcntl_fork();
        if ($pid == -1) {
             die('could not fork');
        } else if (!$pid) {
             // we are the child
            if($output !== null) {
                Artisan::call('resque:listen',['--queue'=>$queue, '--count'=>$count, '--log_expire'=>$expire], $output);
            } else {
                Artisan::call('resque:listen',['--queue'=>$queue, '--count'=>$count, '--log_expire'=>$expire]);    
            }
            die();
        } 
        return $pid;
